...
abstract class AbstractJdbcDao<T : EntityBase, ID : Serializable>
    (
            ...
            protected val referenceDao: ReferenceDao
    ...
    override fun deleteById(id: ID) {
        ...
        //we need to remove references first
        referenceDao.removeReferencesForEntity(id as Long)
    ...
    override fun deleteAll() {
        val type = EntityType.fromTableName(tableName)

        //Firstly, remove references
        referenceDao.removeReferencesForEntityType(type)

        val entityQuery = QueryBuilder.delete(tableName).build()
        val itemsQuery
                = QueryBuilder.delete("Items").where("TypeCode", type.code).build()
    ...
    override fun existsById(id: ID): Boolean {
        val sql = "select count($idColumnName) from $tableName where $idColumnName=?"
        return jdbcTemplate.queryForObject(sql, Int::class.java, id) == 1
    }
}
